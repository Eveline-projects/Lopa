...
class TestTestCaseApi:
    ...
    def test_create_test_case_returns_error_for_invalid_input_data(
        self, problem, payload
    ):
        payload = {
            'problem_id': str(problem.id),
            'input_data': '   ',
    ...
    def test_create_test_case_returns_error_for_invalid_expected_output(
        self, problem
    ):
        payload = {
            'problem_id': str(problem.id),
    ...
    def test_get_test_cases_for_problem_should_return_test_cases_in_order(
        self, problem
    ):
        ...
        assert [item['id'] for item in data] == [
            str(test_case.id) for test_case in expected_order
        ]
        assert [item['input_data'] for item in data] == [
            test_case.input_data for test_case in expected_order
        ]
        assert [item['expected_output'] for item in data] == [
            test_case.expected_output for test_case in expected_order
        ]
